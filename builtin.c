...
/**
 * _exitway - exits the shell
 * @info: Structure containing potential arguments. Used to maintain
 * constant function prototype.
 * Return: exits with a given exit status
 * (0) if info.argv[0] != "exit"
 */
int _exitway(info_t *info)
{
	int exitcheck;

	if (info->argv[1]) /* If there is an exit arguement */
	{
		exitcheck = _geratoi(info->argv[1]);
		if (exitcheck == -1)
{
			info->status = 2;
			print_ermsg(info, "Illegal number: ");
			_eputs(info->argv[1]);
			_eputchar('\n');
			return (1);
		}
		info->err_num = _geratoi(info->argv[1]);
		return (-2);
	}
	info->err_num = -1;
	return (-2);
...
/**
 * _changeproc - changes the current directory of the process
 * @info: Structure containing potential arguments. Used to maintain
 * constant function prototype.
 * Return: Always 0
 */
int _changeproc(info_t *info)
{
	char **arg_array;
